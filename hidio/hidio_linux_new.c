...
void hidio_get_events(t_hidio *x)
{
   ...
   // struct input_event hidio_input_event;
    struct input_event hidio_input_events[64];
...

	{
    		...
    		rd = read (x->x_fd, hidio_input_events, sizeof(struct input_event)*64);
	...
	{
		    if( hidio_input_events[k].type != EV_SYN )
			{
			    for( i=0; i < element_count[x->x_device_number]; ++i )
				{
				    output_element = element[x->x_device_number][i];
				    if( (hidio_input_events[k].code == output_element->linux_code) && \
	                    (hidio_input_events[k].type == output_element->linux_type) )
					{
					    output_element->value = hidio_input_events[k].value;
